...
public class MatrixMath {
    ...
    public static Mat4x4 matrixMakeOrthogonalProjection(float scale) { // int width, int height
        Mat4x4 matrix = new Mat4x4();
        matrix.getM()[0][0] = scale;
        matrix.getM()[1][1] = scale;
        matrix.getM()[2][2] = scale;
        matrix.getM()[3][3] = scale;
        return matrix;
        /*
        // Project Cube Orthographically - Unit Cube Viewport todo ¡esto no va!
        ...
        return matrix;
        /*for ( int i = 0; i < 8; i++ ) {
            projCube[i].x = (2.0f / (right - left)) * worldCube[i].x - ((right + left) / (right - left));
            projCube[i].y = (2.0f / (top - bottom)) * worldCube[i].y - ((top + bottom) / (top - bottom));
            projCube[i].z = (2.0f / (far - near)) * worldCube[i].z - ((far + near) / (far - near));
            projCube[i].x *= -right;
            projCube[i].y *= -top;
            projCube[i].x += right;
            projCube[i].y += top;
